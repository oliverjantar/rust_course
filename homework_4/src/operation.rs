...
impl Csv {
    ...
    #[allow(dead_code)]
    fn from_str(input_str: &str) -> Result<Self, Box<dyn Error>> {
        let mut reader = Reader::from_reader(input_str.as_bytes());
        Self::init(&mut reader)
    }

    fn init<T>(reader: &mut Reader<T>) -> Result<Self, Box<dyn Error>>
    where
        T: std::io::Read,
    {
...
mod tests {
    ...
    #[test]
    fn valid_csv_file_should_print_a_table() {
        {
            let mut file = File::create("test.csv").expect("Unable to create test data.");
...
,Greenland,";
            ...
            file.write_all(data)
                .expect("Error while creating test data");
        }

        let csv = Csv::from_path("test.csv");
        assert!(csv.is_ok());
        println!("{}", csv.unwrap());

        _ = std::fs::remove_file("test.csv");
    }
