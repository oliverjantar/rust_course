...
impl Message {
    ...
    fn serialize(message: &Message) -> Result<Vec<u8>, BincodeError> {
        bincode::serialize(message)
    }

    fn deserialize(data: &[u8]) -> Result<Message, BincodeError> {
        bincode::deserialize(data)
    }

    /// Sends the message to the given stream.
    #[tracing::instrument(name = "Sending message", skip(message, stream))]
    pub fn send_msg(message: &Message, stream: &mut TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    pub fn receive_msg(stream: &mut TcpStream) -> Result<Message, Box<dyn Error>> {
